...
class Nav2RelativeGoalTest(Node):
    def __init__(self, distance_m, timeout_s, output_dir):
        super().__init__("euroboot_nav2_relative_goal_test")
    ...
    def send_goal(self):
        ...
        goal.pose.pose.position.x = p.x + self.distance_m * math.cos(yaw)
        goal.pose.pose.position.y = p.y + self.distance_m * math.sin(yaw)
        goal.pose.pose.position.z = 0.0
        goal.pose.pose.orientation = quat_from_yaw(yaw)
    ...
    def save_and_print(self, status_name):
        ...
        path = self.output_dir / f"nav2_relative_goal_{stamp}.csv"
...
def main():
    ...
    rclpy.init()
    node = Nav2RelativeGoalTest(args.distance, args.timeout, args.output_dir)
    status_name = "UNKNOWN"
    try:
        ...
        if result_future.done():
            ...
            status_name = GoalStatus.STATUS_SUCCEEDED == status and "SUCCEEDED" or f"STATUS_{status}"
        else:
